...
- **Finishing a phase / plan / feature:** the completing reply MUST be in THIS order — (1) the **full checklist in plain English, every item ✓** — each item an outcome a non-technical reader can do/see, NOT a copy of your todo list (no commit hashes, file/function names, "Task N" labels, test counts, or build/`tsc` output — those go in Verification); (2) **What was done** in a plain English paragraph; (3) **Where/how to check it on the front end** — an in-app walkthrough the user can run (nav path + what to click + what they should see), NOT a list of source-file paths; or an explicit "back-end only — here's how to confirm it"; (4) **Verification done** (build/typecheck/lint/tests, for code work); (5) **What's next** (or "Plan complete."). Full format + example → `multi-agent-coordination.md`.
