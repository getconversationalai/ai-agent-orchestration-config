...
- **Finishing a phase / plan / feature:** the completing reply MUST be in THIS order — (1) the **full checklist in plain English, every item ✓** (each item an outcome a non-technical reader understands, not a technical task name); (2) **What was done** in plain English; (3) **Where/how to check it on the front end** — a short walkthrough the user can follow to test it themselves (nav path + what to click + what they should see), or an explicit "back-end only — here's how to confirm it"; (4) **Verification done** (build/typecheck/lint/tests, for code work); (5) **What's next** (or "Plan complete."). Full format + example → `multi-agent-coordination.md`.
