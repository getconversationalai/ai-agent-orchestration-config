...
- **Confirm WHICH TABLE a column lives on — not just that the column name exists somewhere.** After a refactor, columns get *moved* between tables (split a table, denormalize, extract a child entity). The old column name still exists in your memory and in old migration files, but selecting it from its *old* table now silently 400s: PostgREST returns `{ data: null, error: <42703 undefined column> }` with no throw, so a `if (!data) return` swallows it and the feature dies invisibly. This is exactly how reply-flow lost AI replies platform-wide for 43h (a `contact_id` selected from `chat_sessions` after it had moved to `conversation_threads`). For every column you select, verify its **table** against the live schema (`information_schema.columns WHERE table_name = '<table>' AND column_name = '<col>'` or a fresh `pg_dump`), and if the project memory has a **schema-drift / column-moved map** (e.g. reply-flow's `project_code_schema_drift_sweep_*` memory note), read it first — it lists which columns relocated and to where.
